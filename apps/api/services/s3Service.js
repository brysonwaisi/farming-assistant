...
const isConfigured = () => Boolean(REGION && BUCKET && CLOUDFRONT_DOMAIN);
...
const publicUrl = (key) => `https://${CLOUDFRONT_DOMAIN}/${key}`;

// Mint a short-lived presigned PUT URL for a single image upload.
const createUploadUrl = async (contentType, prefix = DEFAULT_PREFIX) => {
  if (!isConfigured()) {
