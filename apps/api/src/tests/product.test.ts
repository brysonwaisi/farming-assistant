...
const sample = (over: Partial<ProductInput> = {}): ProductInput => ({
  title: 'Tomatoes',
  desc: 'Fresh tomatoes',
  img: '/img.jpg',
  categories: ['veggies'],
...
const seed = (over?: Partial<ProductInput>) => Product.create(sample(over));
...
describe('Products', () => {
  describe('reads (public)', () => {
    ...
    it('searches title/desc/category (case-insensitive, regex-safe)', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.map((p: { title: string }) => p.title)).toEqual(['Sweet Banana']);
