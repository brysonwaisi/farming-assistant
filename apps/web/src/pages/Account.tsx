...
// Order documents returned by the orders endpoint (only fields rendered here).
interface Order {
  _id: string;
  createdAt: string;
  products?: unknown[];
...
// Inline edit forms supported by this page (null = read-only view).
type AccountMode = "profile" | null;
...
const Account = () => {
  ...
  const [mode, setMode] = useState<AccountMode>(null); // null | 'profile'
  const [form, setForm] = useState<ProfileForm>({ username: "", email: "" });
  const [saving, setSaving] = useState<boolean>(false);
  const [feedback, setFeedback] = useState<Feedback | null>(null); // { error, message }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await userRequest.get<Order[]>(`/orders/find/${currentUser._id}`);
          ...
          <>
            <AvatarBlock>
              <Avatar
                ...
                onError={(ev) => {
                  ev.currentTarget.src = AVATAR_FALLBACK;
                }}
              />
              <div>
                <input
