...
// The render helper's preloadedState slot. These tests use intentionally partial
// fixtures (e.g. just `currentUser`/`isLoggedIn`), so cast the literals to it.
...
describe("Cart checkout gating", () => {
  it("redirects a guest to login (with return path) instead of opening checkout", async () => {
    renderWithProviders(
      ...
      { preloadedState: { ...cartWithItem, user: { currentUser: null, isLoggedIn: false } } as unknown as PreloadedState },
    );
  ...
  it("renders the cart item and its KES totals", () => {
    renderWithProviders(<Cart />, {
      preloadedState: { ...cartWithItem, user: { currentUser: { _id: "u1" }, isLoggedIn: true } } as unknown as PreloadedState,
    });
  ...
  it("removes the item when its quantity is decremented to zero", async () => {
    ...
    const { store } = renderWithProviders(<Cart />, {
      preloadedState: { ...oneItem, user: { currentUser: null, isLoggedIn: false } } as unknown as PreloadedState,
    });
