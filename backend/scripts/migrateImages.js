...
const { publicUrl, ALLOWED_TYPES, isConfigured } = require('../services/s3Service');

const DRY_RUN = process.argv.includes('--dry-run');
const KEY_PREFIX = 'products/';
const s3 = new S3Client({ region: process.env.AWS_REGION });

const extFor = (contentType) => ALLOWED_TYPES[(contentType || '').split(';')[0].trim()];
...
const uploadToS3 = async ({ body, contentType, ext }) => {
...
const run = async () => {
  ...
  for (const product of products) {
    ...
    try {
      ...
      summary.failed.push({ title: product.title, url, reason: err.message });
      process.stderr.write(`FAILED: ${product.title} (${url}) — ${err.message}\n`);
    }
  }
...
run().catch((err) => {
  process.stderr.write(`Migration aborted: ${err.message}\n`);
  process.exit(1);
});
